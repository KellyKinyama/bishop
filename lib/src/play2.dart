  ...
  if (input.startsWith('moves ')) {
    ...
    switch (subcommand) {
      ...
      default:
        ...
        print('Invalid subcommand \'${parts[1]}\'');
        return;
    }
  }
  if (input == 'resign') {
...
Future<Move?> getAiMove() async {
  if (useEngine) {
    ...
    final res = engine!.monteCarlo(game);
    return res.move;
  } else {
...
void selectUseEngine() {
  ...
  print('Playing against ${useEngine ? 'Engine' : 'Random Mover'}');
}
...
void printCyan(String text) => print(text);
void printYellow(String text) => print(text);
